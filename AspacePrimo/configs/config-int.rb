...
################################################################################
########## START NLM CUSTOM VALUES  ##################################################
occs_db_secret_name = %x[aws ssm get-parameter --name /platform/archivesspace/3-1-4/pDBSecretname |jq --raw-output .Parameter.Value].strip!
occs_db_dbname = %x[aws ssm get-parameter --name /platform/archivesspace/3-1-4/pDBName |jq --raw-output .Parameter.Value].strip!
domain_name = %x[aws ssm get-parameter --name /platform/archivesspace/3-1-4/pDomainname |jq --raw-output .Parameter.Value].strip!

occs_db_engine = %x[aws secretsmanager get-secret-value --secret-id #{occs_db_secret_name} | jq --raw-output .SecretString|jq -r .engine].strip!
occs_db_endpoint = %x[aws secretsmanager get-secret-value --secret-id #{occs_db_secret_name} | jq --raw-output .SecretString|jq -r .host].strip!
occs_db_port = %x[aws secretsmanager get-secret-value --secret-id #{occs_db_secret_name} | jq --raw-output .SecretString | jq -r .port].strip!
occs_db_username = %x[aws secretsmanager get-secret-value --secret-id #{occs_db_secret_name} | jq --raw-output .SecretString |jq -r .username].strip!
occs_db_password = %x[aws secretsmanager get-secret-value --secret-id #{occs_db_secret_name} | jq --raw-output .SecretString |jq -r .password].strip!

########## END NLM CUSTOM VALUES  ##################################################
### Below values are critical when running multiple instances (multi AZ) and to ensure that you're using the SAME cookie prefix per instance

AppConfig[:frontend_cookie_secret] = "7bb697219dc6bdb711d666e191cfc2ac"
AppConfig[:public_cookie_secret]   = "a651f57455a40a456a50e80561ea1802"
AppConfig[:public_user_secret]     = "e35ceae382eee02a113391fa7a7c0846"
AppConfig[:request_user_secret]    = "63ef64441af1d1074337c10768e52cad"
AppConfig[:search_user_secret]     = "63ef64441af1d1074337c10768e52efg"
AppConfig[:staff_user_secret]      = "63ef64441af1d1074337c10768e52xyz"
#
###
...
AppConfig[:db_url] = "jdbc:mysql://#{occs_db_endpoint}:#{occs_db_port}/#{occs_db_dbname}?user=#{occs_db_username}&password=#{occs_db_password}&useUnicode=true&characterEncoding=UTF-8"
...
## set it to something else below.
AppConfig[:solr_url] = "http://127.0.0.1:9000/solr/archivesspace"
#
...
AppConfig[:plugins] << "as_spreadsheet_bulk_updater"
AppConfig[:plugins] << "aspace-oauth"
AppConfig[:plugins] << "export-do"
AppConfig[:plugins] << "pui_help"
AppConfig[:plugins] << "aspace-plugin-primo-main"

#AppConfig[:plugins] = ['aspace-oauth',  'export-do']
#
...
##
AppConfig[:data_directory] = "/var/lib/archspace-data"
#
...
AppConfig[:frontend_proxy_url] = "#{domain_name}/staff"
AppConfig[:public_proxy_url] = "#{domain_name}"
#
...
AppConfig[:authentication_sources] = [
{
       ...
       label: "CAS Sign In",
       config: {
               url: "https://login-prod.nlm.nih.gov",
               host: "login-prod.nlm.nih.gov",
               ssl: true,
               login_url: "/cas/login",
           ...
           fetch_raw_info: -> (s, o, t, user_info) {{email: "#{user_info['user']}@nih.gov"}}
        }
      }
]
#
...
# Use a shared file system (AWS EFS)
AppConfig[:shared_storage] = "/var/lib/archspace-shared/shared"
#
...
## For primo request plugin
AppConfig[:primo_apikey] = "l8xx3b4c565ed5d14da39a27a88ffc53da97"#"l8xx3b4c565ed5d14da39a27a88ffc53da97" #l8xx2a21436fd6274ad19dc6af762b71b440'
AppConfig[:api_base_url] = "https://api-na.hosted.exlibrisgroup.com"
AppConfig[:primo_base_url] = "https://nlm.primo.exlibrisgroup.com" #"https://catalog.nlm.nih.gov"
AppConfig[:inst] = "01NLM_INST"
AppConfig[:vid_view] = "01NLM_INST:01NLM_INST"
